...
shaka.media.StreamInfo.prototype.fetchSegmentInitialization_ = function() {
  ...
  return this.segmentInitializationInfo.fetch().then(shaka.util.TypedBind(this,
      /** @param {!ArrayBuffer} data */
      function(data) {
        var mp4Processor = new uux.Mp4Processor();
        this.segmentInitializationData = shaka.dash.mss.baseUrl ? mp4Processor.generateInitSegment(this) : data;
        //console.log(shaka.util.Uint8ArrayUtils.toBase64(this.segmentInitializationData));
        //uux.download("header_m_" + new Date().getTime() + ".bin", this.segmentInitializationData);
        return Promise.resolve();
      })
  );
};
