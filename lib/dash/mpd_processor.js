...
shaka.dash.MpdProcessor.prototype.createStreamInfo_ = function(
  ...
  if (representation.segmentBase) {
    ...
    ok = this.buildStreamInfoFromSegmentTemplate_(mpd, period, representation, streamInfo);
    streamInfo.duration = representation.duration;
    streamInfo.timescale = representation.timescale;
    streamInfo.trackId = representation.trackId;
    streamInfo.lang = representation.lang;
    streamInfo.type = representation.contentType;
    streamInfo.codecPrivateData = representation.codecPrivateData;
    streamInfo.protected = representation.contentProtections.length > 0;
    streamInfo.contentProtections = representation.contentProtections;
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentTimeline_ =
  ...
  for (var i = 0; i < timeline.length; ++i) {
    ...
    var ref = new shaka.media.SegmentReference(
            startTime,
            scaledStartTime,
            scaledEndTime,
            0 /* startByte */,
    ...
    ref.timescale = representation.segmentTemplate.timescale;
    references.push(ref);
