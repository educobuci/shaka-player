...
shaka.dash.mss.parseMss = function(source, url) {
  ...
  for (var i = 0; i < streamElements.length; i++) {
    ...
    for (var j = 0; j < qualityLevels.length; j++) {
      ...
      //representation.id = (repId++).toString(); //(streamElement.getAttribute('Name') || representation.contentType)  + "=" + representation.bandwidth; // e.g video=1500000
...
shaka.dash.mss.parseStreamQuality = function(streamElement, qualityLevel) {
  ...
  return {
    ...
    contentProtections: [ { pssh: null, schemeIdUri: "urn:uuid:EDEF8BA9-79D6-4ACE-A3C8-27DCD51D21ED", value: null } ],
